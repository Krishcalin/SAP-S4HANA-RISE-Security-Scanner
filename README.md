...
- **Both factors, or no annual figure.** Annualised Loss Exposure is frequency × magnitude, and until 2026-08-31 only the magnitude half was ever calibrated: `contact_frequency` came out of the catalogue for every customer alike, so a 200-person distributor and a 50,000-person bank were both told they are contacted five times a year. On the bundled sample that produced **2.30 expected loss events a year — one every five months — and a mean ALE of 3.4% of revenue, every year, from SAP alone**. Supply `observed_contacts_per_year` (blocked logons, rejected RFC calls, gateway denials, WAF blocks) and the model annualises from your rate; without it the report prices **cost per event** from your figures and **withholds the annual figure**, exactly as it withholds the currency total when nobody has priced the business. *(The keys were withheld rather than zeroed, which is how the HTML report was caught rendering `$0` for an estate with 74 open criticals — a missing number presented as zero risk.)*
- **An incident count cannot calibrate a frequency.** “How many SAP incidents have you had?” is the obvious question and the dangerous one: an organisation with no audit log and no SIEM answers **zero**, truthfully, and a model that believed it would tell the organisations least able to see an attack that they are the safest — using a tool whose own findings say their logging is off. `sap_security_incidents_3y` is therefore a **cross-check, never the frequency**: zero events in three years bounds the true rate at 1.0/year with 95% confidence (the rule of three), and a model above that prints the disagreement beside the figure rather than quietly moving to agree with the weaker measurement.
...
| `--crq-inputs` | Path to the answers file. `{"answers": {"sap_revenue": …, "observed_contacts_per_year": …, "sap_security_incidents_3y": …}}` — loss keys price the magnitude, `observed_contacts_per_year` annualises it |
