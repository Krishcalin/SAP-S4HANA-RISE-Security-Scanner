...
- **838 checks across 39 audit modules.** Measure, never estimate — and know which number you
  ...
  `--modules` choices. Check IDs: **484** are written as literals and **354** appear once the
  ...
  `IAM-<sod rule>` (7). The two sets do not overlap, so they add to 838.
  ...
  It covers all 838 ids — 484 literal plus 354 from six runtime families —
...
### The 39 modules (module key → class → focus)
...
| `osec` | os_security | **OS & infrastructure hardening (D10)**: SAP service-account privilege (sapadm/SAPService&lt;SID&gt; in Administrators, &lt;sid&gt;adm root), Host Agent login shell, /usr/sap & /sapmnt permissions, secure-store owner-only, dangerous host services (telnet/rlogin/NIS). On-prem/hyperscaler only — `not_assessable` in RISE (no OS access) |
