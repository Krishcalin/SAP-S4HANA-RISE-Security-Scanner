...
Somebody takes a set of photographs of how the SAP system is currently configured — lists of users, roles, settings, connections, and so on — and saves them as ordinary files. MonitorRisk reads those files on a computer that has no connection to SAP at all, and applies **620 checks** written by people who know SAP well. It produces a report that says what is wrong, how bad it is, what to do about it, who is able to do it, and what it is worth in money.
...
|---|---|---|
| 1 | **What is wrong?** | 620 checks across 30 subject areas produce a list of findings, each naming the specific accounts, roles, settings or connections involved. |
...
Within each module sit individual rules — 620 in total. Each rule asks one question and, if the answer is unsatisfactory, writes one finding. A finding is a small record, not a sentence: it names the objects involved, so later stages can work with it mechanically.
...
It would have been easier to build the web version separately. It would also have been a serious mistake. Two copies of 620 security rules drift apart: a rule is improved in one and not the other, and eventually the laptop and the console disagree about the same system on the same day. When that happens, the customer stops trusting both.
...
### Why 620 of these are hard

If one check is fifteen lines, 620 checks are not simply nine thousand lines of the same thing. The difficulty is elsewhere:
...
- Improving the guidance for 620 checks becomes an edit to a content file rather than a change to thirty programs, which means it can be done by the person who knows SAP best rather than the person who writes code best.
...
620 checks against a real estate can produce thousands of findings. An undifferentiated list of thousands of problems is not information; it is a way of guaranteeing that nothing gets fixed. This part is about turning the list into decisions.
...
Keeping the words out of the code has three practical effects. The people who write the guidance are SAP specialists, not programmers, and they can improve 620 entries without touching a line of code. The same entry serves the web page, the PDF and the slide deck, so the three cannot drift apart. And when no entry exists yet for a new check, the report falls back to the finding’s own description and a generic remediation — so a missing entry produces a thinner page, never a blank one.
...
Normally you would test a security scanner against a real system with real problems. Here that is almost impossible: you would need a licensed SAP estate, deliberately misconfigured in 620 distinct ways, and you would need it to stay that way.
