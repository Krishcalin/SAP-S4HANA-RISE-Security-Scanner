...
Somebody takes a set of photographs of how the SAP system is currently configured — lists of users, roles, settings, connections, and so on — and saves them as ordinary files. MonitorRisk reads those files on a computer that has no connection to SAP at all, and applies **793 checks** written by people who know SAP well. It produces a report that says what is wrong, how bad it is, what to do about it, who is able to do it, and what it is worth in money.
...
|---|---|---|
| 1 | **What is wrong?** | 793 checks across 36 subject areas produce a list of findings, each naming the specific accounts, roles, settings or connections involved. |
...
If one check is fifteen lines, 793 checks are not simply nine thousand lines of the same thing. The difficulty is elsewhere:
...
- Improving the guidance for 793 checks becomes an edit to a content file rather than a change to thirty programs, which means it can be done by the person who knows SAP best rather than the person who writes code best.
...
793 checks against a real estate can produce thousands of findings. An undifferentiated list of thousands of problems is not information; it is a way of guaranteeing that nothing gets fixed. This part is about turning the list into decisions.
...
>
>  The headline “793 checks” needs a footnote. **448** check identifiers are written out individually in the source. A further **345** come from **six families generated at run time** from shipped rule lists — one check per Web Dispatcher rule (14), one per technical parameter (79), one per code rule (136), one per duty-separation risk (99), one per imported code family (10), one per conflicting-duty pair (7). Both numbers are true; stating both is more useful than picking whichever is larger.
>
