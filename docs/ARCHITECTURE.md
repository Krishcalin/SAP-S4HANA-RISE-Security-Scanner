...
Somebody takes a set of photographs of how the SAP system is currently configured — lists of users, roles, settings, connections, and so on — and saves them as ordinary files. MonitorRisk reads those files on a computer that has no connection to SAP at all, and applies **831 checks** written by people who know SAP well. It produces a report that says what is wrong, how bad it is, what to do about it, who is able to do it, and what it is worth in money.
...
|---|---|---|
| 1 | **What is wrong?** | 831 checks across 36 subject areas produce a list of findings, each naming the specific accounts, roles, settings or connections involved. |
...
Within each module sit individual rules — 685 in total. Each rule asks one question and, if the answer is unsatisfactory, writes one finding. A finding is a small record, not a sentence: it names the objects involved, so later stages can work with it mechanically.
...
It would have been easier to build the web version separately. It would also have been a serious mistake. Two copies of 685 security rules drift apart: a rule is improved in one and not the other, and eventually the laptop and the console disagree about the same system on the same day. When that happens, the customer stops trusting both.
...
The full catalogue runs to **143 logical sources** ([Appendix C](#contents) lists them). Almost nobody supplies all of them on a first run, and that is expected. Every file is optional: the system runs whatever checks it has the information for and records the rest as not assessed.
...
This is the only sensible behaviour when nobody ever supplies all 143 sources — but it creates a serious danger, which the next chapter is entirely about. A system that skips silently produces a short, clean report and looks like good news.
...
**With a coverage statement** — “12 findings. 2 critical.”
  - “You supplied 41 of 143 sources.”
...
If one check is fifteen lines, 831 checks are not simply nine thousand lines of the same thing. The difficulty is elsewhere:
...
- Improving the guidance for 831 checks becomes an edit to a content file rather than a change to thirty programs, which means it can be done by the person who knows SAP best rather than the person who writes code best.
...
831 checks against a real estate can produce thousands of findings. An undifferentiated list of thousands of problems is not information; it is a way of guaranteeing that nothing gets fixed. This part is about turning the list into decisions.
...
Keeping the words out of the code has three practical effects. The people who write the guidance are SAP specialists, not programmers, and they can improve 685 entries without touching a line of code. The same entry serves the web page, the PDF and the slide deck, so the three cannot drift apart. And when no entry exists yet for a new check, the report falls back to the finding’s own description and a generic remediation — so a missing entry produces a thinner page, never a blank one.
...
Normally you would test a security scanner against a real system with real problems. Here that is almost impossible: you would need a licensed SAP estate, deliberately misconfigured in 685 distinct ways, and you would need it to stay that way.
...
>
>  The headline “831 checks” needs a footnote. **478** check identifiers are written out individually in the source. A further **353** come from **six families generated at run time** from shipped rule lists — one check per Web Dispatcher rule (14), one per technical parameter (79), one per code rule (139), one per duty-separation risk (99), one per imported code family (10), one per conflicting-duty pair (7). Both numbers are true; stating both is more useful than picking whichever is larger.
>
...
The counts this document states — 831 checks, 478 written as literals, 353 generated at run time from 6 rule families, 39 modules, 135 custom-code rules, 27 duty-separation risks, 78 profile parameters and 143 logical sources — are derived from the code by `tests/test_architecture_doc.py`, which fails the build if this page and the source disagree. Edition 1.1 corrected three figures that had drifted from the source.
