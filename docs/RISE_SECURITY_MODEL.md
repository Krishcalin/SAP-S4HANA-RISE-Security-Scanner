...
| **Code Vulnerability Analyzer (CVA)** | **Free for PCE** via remote ABAP Test Cockpit on BTP **[SAP-primary]** | **DO NOT BUILD ABAP SOURCE SCANNING.** Mature, always current, runs where the code lives. Consume the *outcome*: check whether CVA/ATC is switched on and being run — Cloud ALM already exposes a CVA status store — and flag "custom code security not enabled" as a governance finding. One check, not a product line. |
