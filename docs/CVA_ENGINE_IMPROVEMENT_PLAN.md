...
> **All twelve resolved: ten settled, one partially, one declined.** U1, U2, U4, U5, U6, U7, U9, U10, U11 and U12 are settled; U3 is partially settled (the released spelling is confirmed, the legacy-alias question is not answerable from a source that gives examples rather than signatures); U8 is declined, because no fetchable SAP source names the interface and writing it from memory is the failure this list exists to prevent against SAP's ABAP keyword documentation
...
| ~~U7~~ | **SETTLED — real, and B7's shape exactly.** SAP's syntax is `READ DATASET dset INTO dobj [MAXIMUM LENGTH mlen] [[ACTUAL] LENGTH alen].` and the effect is stated plainly: "This statement exports data from the file specified in `dset` to the data object `dobj`." An INTO target filled from outside — and a file on the application server was written by an interface, an upload, another system or an operator, never by the program reading it. `READ DATASET` is a KEYWORD, so it needs no entry in `CONFIRMED_SOURCE_IDENTIFIERS`; that table is for library names. | `abapread_dataset.htm`, AS ABAP 7.55 | ✅ |
| ~~U8~~ | **DECLINED — the decision is settled even though the question is not.** Seeding a handler's request parameter is ordinary B2-shaped work needing no new machinery. What it needs is the interface and method name, and neither appears in ANY SAP file this project can fetch: the released-classes listing contains no `IF_*HTTP*` interface at all, the ABAP-for-Cloud-Development cheat sheet does not mention one, and the keyword documentation has no page under the names tried. Writing the identifier from memory is precisely the failure U10 exists to prevent — and remembering it correctly would not make it verified. Recorded beside B9's refusal with what would settle it: the interface's page on the SAP Business Accelerator Hub, or an SAP-published sample implementing a handler. | not answered by any fetchable source | ⛔ |
