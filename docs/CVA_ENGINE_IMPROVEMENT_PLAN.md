...
| U10 | **The JSON-deserialisation, dynpro-field-read and F4-help identifiers proposed for `_SOURCE_RE`** (B3, B5) | The *regexes* were validated against those strings; the *identifiers themselves* were not grep-confirmed in a fetched SAP file. Under the repo's own no-fabrication rule they must not ship unconfirmed. | `21_XML_JSON.md` and `18_Dynpro.md` — grep before adding. |
| U11 | **`FILTER` and `FOR` as WHERE-bearing internal-table constructs** (A10 alternation) | Named alongside the covered constructs, but no dynamic parenthesised form was verified for either. | the `FILTER` section of `31_WHERE_Conditions.md`. |
| U12 | **The namespace-authority hosts in F5's allowlist beyond the SAP and W3C entries** | Ship it as a maintained allowlist with that stated in a comment — not as a claim about SAP. | n/a — this is our list, and should be labelled as such. |
