...
**437** check ids are written as literals in `modules/`, across **37** modules. A further **282** are built at runtime from shipped rule tables, giving **719** in total.
...
- **523 of 719** checks answer no Baseline requirement — **which is not a failure.** Segregation of duties, GRC, financial controls, the attack-path content and the RISE-specific checks have no Baseline equivalent, and that is where this product goes beyond it.
